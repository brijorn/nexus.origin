...
module.exports.run = async (bot: OriginClient, message: Message, args: string[], guild: GuildSettings) => {
	if (!args[0]) {
		const help = bot.commands.get('weather');
		const helpe = new MessageEmbed()
			.setTitle('Weather')
			.setDescription(help!.description + `\n\n\`${help!.aliases.map((e: any) => `${e}`).join(', ')}\``)
