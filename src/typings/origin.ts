...
export class VerificationSettings {
	...
	public async get(guildId: any) {
		return new FetchedVerification(
			await db
				.withSchema("modules")
				.table("verification")
	...
	public async default(
		...
		verifiedRole?: string | bigint,
		unverified_role?: string | bigint
	) {
		const settings = new FetchedVerification(
			await db
				.withSchema("modules")
				.table("verification")
				.returning("*")
...
export class FetchedVerification extends VerificationSettings {
	...
	public async update(setting: (keyof this), value: updateTableData) {
		return new FetchedVerification(
            await updateTable('verification', 'modules', this.guild_id as string, value)
		);
	}
}
...
export class Panel {
	...
	async get(opt?: GetQueryOptions) {
		const panel = new FetchedPanel(
			await getTable("interfaces", "ticketing", opt as any)
		);
		return panel;
	}
...
export class Ticket {
	...
	public async create(guildId: string, panel: FetchedPanel, information: any) {
		if (panel.claim_system === true) {
			return new FetchedTicket(await db.withSchema("ticketing").table(""));
		} else {
			return new FetchedClaimTicket(
				await db.withSchema("ticketing").table("awaiting_claim").insert({
					guild_id: guildId,
				})
			);
		}
...
export class FetchedApplication extends ApplicationSettings {
	...
	public async update(setting: keyof this, data: updateTableData) {
		const settings = new FetchedApplication(
			await updateTable("verification", "modules", this.guild_id as string, data)
		);
		return settings;
	}
}
...
export class GuildSettings {
	...
	async create(guildId: string) {
		...
		const guild: this = await insertTable(
			"guild",
			"public",
			guildId,
			defaultData
...
async function updateTable(
	...
	schema: string,
	guildId: string,
	query?: updateTableData
) {
    const where = (query && query.where) ? { guild_id: guildId, ...query.where } : { guildId: guildId }
	const info = await db
		.withSchema(schema)
        .table(tableName)
        .where('guild_id', '=', guildId)
        .where(where)
        .first()
        .update((query!.fields === 'all') ? query!.data : where)
        .returning("*");
	return info as any;
}
