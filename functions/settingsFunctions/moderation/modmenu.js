...
module.exports = async (bot, message, args, guild) => {
    ...
    if (args[1].toLowerCase().includes('modlog')) {
        const parseChannel = require('../../../lib/channelparse')
        let start = undefined
        if (!args[2]) {
    ...
    if(args[1].toLowerCase() === 'mutedrole') {
        const roleparse = require('../../../lib/role')
        const res = roleparse(bot, message, args, guild,  guild, 'moderation', 'mutedrole', 'MutedRole', 'string')
