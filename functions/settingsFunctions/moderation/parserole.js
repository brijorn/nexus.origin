module.exports = async (message, i, roles, ele, newroles, notfound, createdRoles) => {
	...
	if (isNaN(ele) === true) {
		...
		else if (message.guild.roles.cache.find(arole => arole.name === roles[i])) {
	...
	else if (!await message.guild.roles.cache.get(ele)) {notfound.push('here:', roles.indexOf(ele) + ele);}
	else {
		roleid = ele,
		status = 'found';
	}
