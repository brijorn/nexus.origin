...
module.exports.run = async (bot, message, args, guild) => {
    ...
    if (moderrors.moderationRoles() === false) return
     ...
     let time = parse.member(args, args)
     args = time.newarr
    ...
    try {
        await user.roles.add(mod.muteRole)
    }
    catch {
        muted = false
        if (!message.guild.roles.cache.find(mod.muteRole)) return message.channel.send(embed('none', `I could not find the given mute role.`, guild, failure))
        else return message.channel.send(embed('none', 'Failed to mute the user, be sure I have the sufficient permissions, you can trust me :)', guild, failure))
    }
    ...
    await moderate.userUpd.updateUser(message, guild, user, 'mute', muteObj, time)
}
