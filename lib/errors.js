...
module.exports = async (message, guild) => {
    ...
    const CheckFor = {
        all: function(sendmsg=true) {
            if (!message.author.id === message.guild.owner.id && !guild.permissions.owners.includes(message.author.id) && !guild.permissions.admins.includes(message.author.id)) {
            ...
            for (i=0; i < modroles.length; i++) {
                ...
                if (role.startsWith('greaterthan')) {
                    ...
                    if (message.member.roles.cache.find(e => {
                        ...
                        if (!message.member.roles.cache.find(method)) {
                            return
                        }
                        else {
                            return clear = true
